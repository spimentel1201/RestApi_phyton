from flask import Flask, Resource, Api
...
class Users(Resource):
    # metodos aqui
    def get():
        data = pd.read_csv('users.csv') # leer CSV
        data = data.to_dict() # convertir dataframe a dictionary
        return {'data': data}, 200 # retornar data y 200 OK code

    def post(self):
        parser = reqparse.RequestParser() # inicializar
        parser.add_argument('userId', required=True) # adicionar args
        parser.add_argument('name', required=True)
        parser.add_argument('city', required=True)
        args = parser.parse_args() # parsear argumentos al dictionary
        # crear nuevo dataframe almacenando nuevos valores
        ...
        # adicionar los nuevos valores recibidos
...
api.add_resource(Users, '/users') # '/users' es nuestro entry point para Users
api.add_resource(Locations, '/locations') # and '/locations' es nuestro entry point para Locations
if __name__ == '__main__':
    app.run() # ejecutar nuestra Flask app
